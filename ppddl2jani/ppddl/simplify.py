...
class VarValueRenaming(object):
    ...
    def apply_to_task(self, task, filter_unimportant_ops=True):
        if DEBUG:
            self.dump()
        ...
        self.apply_to_operators(task.operators, filter_unimportant_ops)
    ...
    def apply_to_operators(self, operators, filter_unimportant_ops=True):
        new_operators = []
        num_removed = 0
        for op in operators:
            new_op = self.translate_operator(op, filter_unimportant_ops)
            if new_op is None:
                num_removed += 1
                if DEBUG:
    ...
    def translate_operator(self, op, filter_unimportant_ops=True):
...
def filter_unreachable_propositions(sas_task, filter_unimportant_ops=True):
    ...
    renaming.apply_to_task(sas_task, filter_unimportant_ops)
